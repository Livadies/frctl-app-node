...
FRCTL Node — локальный доверенный компонент FRCTL Hub для запуска заранее разрешённых клиентов на компьютере пользователя. Он работает только на `127.0.0.1`, показывает системное окно подтверждения перед каждым запуском и сохраняет локальный журнал с SHA-256-цепочкой целостности.
